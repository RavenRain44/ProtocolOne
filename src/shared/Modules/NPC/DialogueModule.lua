...
function DialogModule.new(npcName, npc, prompt, animation)
	...
	self.animArrowText = tweenService:Create(self.npcGui.arrow, TweenInfo.new(0.3), { TextTransparency = 1 })
	self.animArrowStroke = tweenService:Create(self.npcGui.arrow.UIStroke, TweenInfo.new(0.3), { Transparency = 1 })
			...
			local enterCon = option.MouseEnter:Connect(function()
				tweenService
					:Create(
						...
						{ Size = UDim2.new(option.Size.X.Scale + (option.Size.X.Scale * 0.05), 0, 0.4, 0) }
					)
					:Play()
				tweenService
					:Create(
...
function DialogModule:showGui()
	...
	coroutine.wrap(function()
		...
		if self.npcGui.name.TextTransparency ~= 1 then
			return
		end -- check if already chose an opiton
